...
class Attention(nn.Module):
    def __init__(self, dim,
                 ...
                 attn_drop_ratio=0.,
                 proj_drop_ratio=0.,
                 k_value=1.0):  # Introducing the k value for Chebyshev's inequality
        ...
        self.k_value = k_value  # Chebyshev's k value
    ...
    def forward(self, x, e):
        ...
        # @: matrix multiplication -> [batch_size, num_heads, num_patches, num_patches]
        ...
        std_v = torch.sqrt(((x - x.mean(dim=1, keepdim=True)) ** 2).mean(dim=1, keepdim=True))  # sigma_v

        # Apply Chebyshev's inequality bounds
...
class DownSample(nn.Module):
    def __init__(self, embed_dim=48, grid_size=64):
...
class UpSample(nn.Module):
    def __init__(self, embed_dim=64, grid_size=64):
...
class PixelShuffleLast(nn.Module):
    def __init__(self, in_c, out_c, grid_size, scale_factor=4):
