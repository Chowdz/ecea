...
class Trainer(nn.Module):
    def __init__(self, batch_size, epoch, n_epoch, lr, beta1, beta2, img_size, in_c, out_c,
        ...
        self.l1_loss_cshe = nn.L1Loss().to(self.device)
    ...
    def forward(self, dataset):
        ...
        dataloader = DataLoader(dataset=dataset, batch_size=self.batch_size, shuffle=True, drop_last=True, num_workers=0)
        self.load_net(self.epoch - 1)
        for epoch in range(self.epoch, self.n_epoch):
            dataset.shuffle_masks()
            for index, x in enumerate(dataloader):
                ...
                cshe_loss = self.l1_loss_cshe(cshe_fake, cshe) * self.cshe_loss_weight
                ...
                if batches_done % self.sample_interval == 0:
                    ...
                    cshe_img = torch.split(cshe, split_size_or_sections=1, dim=0)
